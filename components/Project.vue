    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <input class="form-control" id="name" type="text" placeholder="Enter project name"
                       v-model="data['name']"/>
            </div>

            <div class="form-group">
                <label for="url-check">Validation request URL</label>
                <input class="form-control" id="url-check" type="text"
                       placeholder="Enter URL for send validation request" v-model="data['url_check_account']"/>
            </div>

            <div class="form-group">
                <label for="url-notify">Payment notify request URL</label>
                <input class="form-control" id="url-notify" type="text"
            ...
            <div class="form-group">
                ...
                <input class="form-control" id="create-order-allowed-urls" type="text"
                       placeholder="Enter create order allowed URLs" v-model="createInvoiceAllowedUrls"/>
                <small id="create-order-allowed-urls-help" class="form-text text-muted">
                    Request for creation order will be possible only from the URLs specified here. List URLs through
                    comma.
                </small>
            </div>

            <div class="form-group">
                <label for="url-redirect-success">URL for redirect user after successfully completed payment</label>
                <input class="form-control" id="url-redirect-success" type="text"
            ...
            <div class="form-group">
                ...
                <input class="form-control" id="url-redirect-fail" type="text"
            ...
            <div class="form-group">
                ...
                <v-select class="form" v-bind:options="currencies"
                          v-bind:placeholder="'Select callback request currency'" @search="onCurrencySearch"
                          v-bind:onChange="onChangeCallbackCurrency" v-model="callbackCurrency"
                          v-bind:filterable="false"></v-select>
                <small id="callback-currency-help" class="form-text text-muted">
                    If the currency is not selected, the value for callback requests will be taken from the merchant's
                    currency settings.
                </small>
            </div>
            ...
            <div class="form-group">
                ...
                <v-select class="form" v-bind:options="currencies" v-bind:placeholder="'Select limits currency'"
                          @search="onCurrencySearch" v-bind:onChange="onChangeLimitsCurrency" v-bind:filterable="false"
                          v-model="limitsCurrency"></v-select>
                <small id="limit-currency-help" class="form-text text-muted">
                    If the currency is not selected, the currency for min and max payment amount will be taken from the
            ...
            <div class="form-group">
                <div class="row">
                    <div class="col">
                        ...
                        <input class="form-control" id="min-payment-amount" type="text"
                               placeholder="Enter min payment amount" @keyup="formatAmount('min_payment_amount')"
                               v-model="data['min_payment_amount']"/>
                        <small class="form-text text-muted">
                            Amount in currency of limit. Decimal separator is dot ("."). Possible 2 digit after decimal
                            separator. For example: 10.23
                        </small>
                    </div>
                    <div class="col">
                        <label for="max-payment-amount">Max amount allowed to payment</label>
                        <input class="form-control" id="max-payment-amount" type="text"
                               placeholder="Enter max payment amount" @keyup="formatAmount('max_payment_amount')"
            ...
            <div class="form-group">
                <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="send-email-notify"
                           v-model="data['send_notify_email']">
                    <label class="form-check-label" for="send-email-notify">Notify about successful payments via
                        email</label>
                </div>
            </div>
            ...
            <div class="form-group">
                <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="only-fixed-packages"
                           v-model="data['only_fixed_amounts']">
                    <label class="form-check-label" for="only-fixed-packages">Allow payments only fixed amount
                        packages</label>
                </div>
            </div>
            ...
            <div class="form-group">
                <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="is-allow-dynamic-notify-urls"
                           v-model="data['is_allow_dynamic_notify_urls']">
                    <label class="form-check-label" for="is-allow-dynamic-notify-urls">Allow send validation url and
                        notification url in the order create request</label>
                </div>
            </div>

            <div class="form-group">
                <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="is-allow-dynamic-redirect-urls"
                           v-model="data['is_allow_dynamic_redirect_urls']">
                    <label class="form-check-label" for="is-allow-dynamic-redirect-urls">Allow send user's redirect urls
                        in the order create request</label>
                </div>
            </div>

            <div class="form-group" v-if="data['send_notify_email'] === true">
                <label for="notify-emails">Emails for payments notifications</label>
                <input class="form-control" id="notify-emails" type="text"
                       placeholder="Enter emails for payments notifications" v-model="notifyEmails"/>
                <small id="notify-emails-help" class="form-text text-muted">
                    On this emails list will be send notifications about successfully completed payments. List URLs
                    through comma.
                </small>
            </div>
                                    ...
                                    <div class="form-group">
                                        <div class="row">
                                            <div class="col">
                                                <label>Price</label>
                                                <input class="form-control" type="text"
                                                       placeholder="Enter fixed package price" v-model="item['price']"/>
                                                <small class="form-text text-muted">
                                            ...
                                            <div class="col">
                                                ...
                                                <v-select class="form" v-bind:options="currencies"
                                                          ...
                                                          @search="onCurrencySearch" v-bind:filterable="false"
                                                          v-model="item['currency_int']"></v-select>
                                                <small class="form-text text-muted">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            removeFixedPackage: function (index) {
            ...
            formatAmount: function (field) {
                ...
                let match = (/(\d{0,9})[^.]*((?:\.\d{0,2})?)/g).exec(this.data[field].replace(/[^\d.]/g, ''));
                this.data[field] = match[1] + match[2];
            },
            onChangeCallbackCurrency: function (val) {
                if (val === null) {
            ...
            onChangeLimitsCurrency: function (val) {
                if (val === null) {
            ...
            save: function () {
                ...
                for (let key in this.data['fixed_package']) {
                    ...
                    for (let i = 0; i < this.data['fixed_package'][key].length; i++) {
