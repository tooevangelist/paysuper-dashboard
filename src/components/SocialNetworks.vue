...
export default {
  ...
  async mounted() {
    const { data } = await axios.get(`${process.env.VUE_APP_P1AUTH_URL}/oauth/sources/`);

    for (let i = 0; i < data.length; i += 1) {
      if (this.socialNetworksIcons[data[i].name] !== true) {
        // eslint-disable-next-line
        continue;
      }

      data[i].url = process.env.VUE_APP_P1AUTH_URL + data[i].url;
      data[i].icon = this.socialNetworksIcons[data[i].name];
    }
