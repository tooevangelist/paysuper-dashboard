...
export default {
  ...
  methods: {
    ...
    ...mapActions('WebHookTest', ['fetchVirtualItems']),
...
<template>
  <div>
    <div v-if="get(virtualItems, 'count', 0) > 0">
      ...
      <div class="items">
        ...
        <div class="items__row" v-for="(item, index) in items" :key="index">
          <UiSelect
            v-model="items[index]"
            :options="virtualItemsList"></UiSelect>
        </div>

        <div class="items__add" @click="addItem" v-if="items.length < virtualItemsList.length">
          <IconPlus></IconPlus> Add item
        </div>
      </div>
    ...
    <NoResults type="add-new" v-else>
      Please
      <router-link :to="`/projects/${projectId}/virtual-items/`">
