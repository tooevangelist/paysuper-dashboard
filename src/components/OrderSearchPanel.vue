<template>
  <div class="search-container">
    <button
      type="button"
      class="btn btn-outline-success btn btn-sm btn-filters"
      :class="{ 'hide': !isShowed }"
      @click="isShowed = !isShowed"
    >
      <i
        class="fa"
        :class="{ 'fa-angle-double-down': !isShowed, 'fa fa-angle-double-up': isShowed }"
      ></i>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      if (this.values.country != null && Array.isArray(this.values.country)) {
        ...
        for (let i = 0; i < this.values.country.length; i += 1) {
      ...
      if (this.values.project != null && Array.isArray(this.values.project)) {
        ...
        for (let i = 0; i < this.values.project.length; i += 1) {
      ...
      if (this.values.paymentMethod != null && Array.isArray(this.values.paymentMethod)) {
        ...
        for (let i = 0; i < this.values.paymentMethod.length; i += 1) {
      ...
      if (this.values.status != null && Array.isArray(this.values.status)) {
        ...
        for (let i = 0; i < this.values.status.length; i += 1) {
  ...
  mounted() {
    ...
    if (Object.keys(query).length > 0 && query.pm_date_from && query.pm_date_to) {
      const filters = {
